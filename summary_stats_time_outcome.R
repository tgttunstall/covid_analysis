...
gp_stats = groupedstats::grouped_summary(
  data = lf,
  grouping.vars = c(mediator, timepoint, outcomes),
  measures = value,
...
#******************************
gp_gmean = lf %>%
  group_by(mediator, timepoint, outcomes) %>%
...
for (i in common_cols){
  ...
  if( all.equal(gp_gmean_df[i], gp_stats_df[i]) ){
    print('PASS')
    expected_rows = nlevels(factor(gp_gmean_df$mediator)) * nlevels(factor(gp_gmean_df$timepoint)) * nlevels(factor(gp_gmean_df$outcomes))
  } else{
...
# rearrange columns
my_col_order = c("mediator" ,"timepoint", "outcomes"
...
# rename cols
new_colnames = c("mediator" ,"timepoint", "outcomes"
